...
public class GuiWidget {
    ...
    public void renderOverlay(CustomizeGuiOverlayEvent event) {
        ...
        if (this.minecraft.player != null) {
            ...
            //if (overlay.id().equals(VanillaGuiOverlay.TEXT.id())) if (event.getOverlay() != ForgeGui) return;

            GlStateManager._clearColor(1.0F, 1.0F, 1.0F, 1.0F);

            // Render different parts of the HUD
            this.getWidgetBase(player, event);
            this.getPlayerHealthBar(player, event);
            this.getPlayerAirBar(player, scaled, event);
            this.getFoodValue(player, event);
            this.getSatuValue(player, event);
            this.getArmorValue(player, event);

            // Conditionally render additional HUD elements based on configuration
            if (ReignitedHudConfig.DURABILITY.get()) this.getDurability(player, event);
            if (ReignitedHudConfig.MOUNT.get()) this.getMountInfo(player, event);
            if (ReignitedHudConfig.EFFECT.get()) this.getEffects(player, scaled, event);
            if (ReignitedHudConfig.TIME.get()) this.getClock(scaled, event);
        }
    ...
    private void getWidgetBase(LocalPlayer player, CustomizeGuiOverlayEvent event) {
        ...
        event.getGuiGraphics().blit(ReignitedHudID.TEX_HUD_BAR, 13, 13, 227, 0, 5, 25);

        // Render dynamic HUD element based on player's experience progress
        event.getGuiGraphics().blit(ReignitedHudID.TEX_HUD_BAR, 14, 14, 223, 1, 3, 23 - (int)(player.experienceProgress  * 23.0F));

        // Render HUD base
        //RenderSystem.setShaderTexture(0, ReignitedHudID.TEX_HUD_BASE);
        event.getGuiGraphics().blit(ReignitedHudID.TEX_HUD_BASE, 15, 11, 0, 0, 29, 29);

        // Render player's name on the HUD
        RenderDrawCallback.drawFontWithShadow(event, player.getName().getString(), 48, 13, 16777215);
        ...
        RenderDrawCallback.drawFontBoldCentered(event, enchantedPoints, 30, 35, 13172623, 2957570);
        ...
        if(this.minecraft.level != null) {
            if (this.minecraft.level.getDifficulty() == Difficulty.HARD) {
                ...
                RenderDrawCallback.drawIcon(ReignitedHudID.TEX_HUD_ICON, event, 25, 11, 4, 2);
            }
        }
    } // getWidgetBase ()
     ...
     */
    private void getPlayerAirBar(LocalPlayer player, Window scaled, CustomizeGuiOverlayEvent event) {
        ...
        if (player.isUnderWater()) {
            ...
            RenderSystem.setShaderTexture(0, ReignitedHudID.TEX_HUD_BAR);
            RenderDrawCallback.drawLongBar(ReignitedHudID.TEX_HUD_BAR, event, posX, posY, 5, air / 300.0F);
            ...
            if (player.getAirSupply() < player.getMaxAirSupply())
                event.getGuiGraphics().blit(ReignitedHudID.TEX_HUD_ICON,screenWidth / 2 - 4, posY - 2, texX, 0, 10, 10);
        }
     ...
     * @param player The client player entity
     */
    private void getMountInfo(LocalPlayer player, CustomizeGuiOverlayEvent event) {
        if (player.getVehicle() instanceof LivingEntity) {
            ...
            if (mount instanceof Horse && ((Horse)mount).isTamed()) {
                event.getGuiGraphics().blit(ReignitedHudID.TEX_HUD_BASE,15, 56, 0, 29, 6, 25);
            } else {
                event.getGuiGraphics().blit(ReignitedHudID.TEX_HUD_BASE,15, 56, 6, 29, 6, 19);
            }

            // Display the icon texture
            //RenderSystem.setShaderTexture(0, ReignitedHudID.TEX_HUD_ICON);
            RenderDrawCallback.drawIcon(ReignitedHudID.TEX_HUD_ICON, event, 23, 57, 1, 10);

            // Display the health bar texture
            //RenderSystem.setShaderTexture(0, ReignitedHudID.TEX_HUD_BAR);
            RenderDrawCallback.drawMediumBar(ReignitedHudID.TEX_HUD_BAR, event, 23, 68, 1, health);
            ...
            if (mount instanceof PlayerRideableJumping && ((Horse)mount).isTamed())
                RenderDrawCallback.drawMediumBar(ReignitedHudID.TEX_HUD_BAR, event, 23, 74, 5, player.getJumpRidingScale());

            // Display the mount's name on the HUD
            String mountStat = mount.getDisplayName().getString();
            RenderDrawCallback.drawFontWithShadow(event, mountStat, 35, 58, 16777215);
        }
    ...
    private void getFoodValue(LocalPlayer player, CustomizeGuiOverlayEvent event) {
        ...
        RenderDrawCallback.drawIcon(ReignitedHudID.TEX_HUD_ICON, event, 47, 31, 1, icon);
        ...
        RenderDrawCallback.drawFontWithShadow(event, hunger, 59, 32, color, shadow);
    } // getFoodValue ()
     ...
     */
    private void getSatuValue(LocalPlayer player, CustomizeGuiOverlayEvent event) {
        ...
        // Draw hunger icon
        RenderDrawCallback.drawIcon(ReignitedHudID.TEX_HUD_ICON, event, posXIcon, 31, 1, icon);
        ...
        RenderDrawCallback.drawFontWithShadow(event, saturation, posXtext, 32, color, shadow);
     ...
     * @param player The player entity for which to update the HUD
     */
    private void getArmorValue(LocalPlayer player, CustomizeGuiOverlayEvent event) {
        ...
        if (armor > 0) {
            //RenderSystem.setShaderTexture(0, ReignitedHudID.TEX_HUD_ICON);
            RenderDrawCallback.drawIcon(ReignitedHudID.TEX_HUD_ICON, event, posXicon, 31, 1, 8);
            RenderDrawCallback.drawFontWithShadow(event, armor + "", posXtext, 32, 12106180, 1579034);
        }
        ...
        if (player.getAttributeValue(Attributes.ARMOR) > 0.0) {
            ...
            //RenderSystem.setShaderTexture(0, ReignitedHudID.TEX_HUD_ICON);
            RenderDrawCallback.drawIcon(ReignitedHudID.TEX_HUD_ICON, event, posXicon, 31, 1, 9);
            RenderDrawCallback.drawFontWithShadow(event, toughness + "", posXtext, 32, 12106180, 1579034);
        }
     ...
     */
    private void getDurability(LocalPlayer player, CustomizeGuiOverlayEvent event) {
     ...
     */
    private void getEffects(LocalPlayer player, Window scaled, CustomizeGuiOverlayEvent event) {
        ...
        if (!collection.isEmpty()) {
            ...
            for (MobEffectInstance potionEffect : Ordering.natural().reverse().sortedCopy(collection)) {
                MobEffect potion = (MobEffect) potionEffect.getEffect();
                ...
                if (potionEffect.showIcon()) {
                    ...
                    GlStateManager._clearColor(1.0F, 1.0F, 1.0F, 1.0F);
                    //RenderSystem.setShaderTexture(0, ReignitedHudID.TEX_HUD_BASE);
                    event.getGuiGraphics().blit(ReignitedHudID.TEX_HUD_BASE, posX, posY, 88, 0, 29, 21);
                    GlStateManager._clearColor(1.0F, 1.0F, 1.0F, transparency);
                    //RenderSystem.setShaderTexture(0, ReignitedHudID.TEX_HUD_EFFECT);
                    event.getGuiGraphics().blit(ReignitedHudID.TEX_HUD_EFFECT, posX + 6, posY - 3, icon % 14 * 18, icon / 14 * 18, 18, 18);

                    RenderDrawCallback.drawFontBoldCentered(event, duration, posX + 15, posY + 10, potion.getColor(), 0);
                }
            }
        }
     ...
     */
    private void getClock(Window scaled, CustomizeGuiOverlayEvent event) {
        ...
        //RenderSystem.setShaderTexture(0, ReignitedHudID.TEX_HUD_BASE);
        event.getGuiGraphics().blit(ReignitedHudID.TEX_HUD_BASE, screenWidth / 2 - 22, screenHeight - 36, 165, 0, 44, 14);

        //RenderSystem.setShaderTexture(0, ReignitedHudID.TEX_HUD_ICON);
        RenderDrawCallback.drawIcon(ReignitedHudID.TEX_HUD_ICON, event, screenWidth / 2 - 19, screenHeight - 33, row, icon);

        RenderDrawCallback.drawFontWithShadowCentered(event, hourdisplay + ":" + minutedisplay, screenWidth / 2 + 5, screenHeight - 32, color, 0);
    } // getClock ()
